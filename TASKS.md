...
- [x] Orphan foto temizliği: `WizardStep5Photos.vue`'daki `removePhoto()` şu an yalnızca formdan kaldırıyor, Supabase Storage'daki dosyayı silmiyor (bilinen eksik, kod içinde not var) — `removePhoto()`'ya gerçek `supabase.storage.from('location-photos').remove([path])` çağrısı eklenerek düzeltilecek — general-purpose (2026-07-27 otonom oturumda tamamlandı: public URL'den storage path'i çıkaran `storagePathFromPublicUrl()` helper eklendi, `removePhoto()` async oldu — önce formdan kaldırır sonra storage'dan siler, hata olursa yalnızca `console.warn` ile yutulur, UI'ı bloklamaz; `wizard.photos` şekli `string[]` olarak korundu (Step6'nın `photo_urls` insert'i buna bağımlı); `~/kampla-verify` kopyada typecheck 1 baseline hata (tailwind.config.ts, ilgisiz) dışında temiz, `nuxt build` başarılı, `/konum-ekle/5` HTTP 200; gerçek Supabase silme çağrısı sandbox'ta canlı test edilemedi)
...
- [x] mobile-web statik sayfalarını (`hakkinda`, `kullanim-kosullari`, `gizlilik`) admin panelinden düzenlenebilen `site_content` tablosundan okuyacak şekilde bağla (Faz 8'de not düşülmüştü, admin `/icerik` sayfası zaten bu tabloya yazıyor ama mobile-web hâlâ statik i18n metni gösteriyor) — tabloda satır yoksa/fetch başarısızsa mevcut i18n metnine fallback (mevcut ortamlarda migration 0013 uygulanmamış olabilir, sayfa asla boş kalmamalı) — general-purpose (2026-07-27 otonom oturumda tamamlandı: yeni `composables/useSiteContent.ts` — `key`+aktif locale ile sorgu, yoksa `lang='tr'`e düşer, herhangi bir hata/istisna `content.value=null` bırakır; 3 sayfa `onMounted`'da fetch edip DB varsa onu, yoksa orijinal statik i18n metnini render ediyor (ilk render her zaman statik metin, DB gelince değişir) — "never blank" yapısal olarak garanti; kullanim-kosullari/gizlilik'teki taslak uyarı kutusu kaynaktan bağımsız her zaman gösteriliyor; admin `/icerik` ve migration dosyasında yalnızca yorum düzeltmesi yapıldı, mantık değişmedi; `~/kampla-verify` kopyada mobile-web+admin ikisinde de typecheck 1 baseline hata dışında temiz, `nuxt build` başarılı, 3 route HTTP 200; canlı Supabase ile DB-hit render'ı sandbox'ta görsel doğrulanamadı, kod yolu üzerinden akıl yürütüldü)
