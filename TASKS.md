...
- [ ] Manuel Konum Ekle/Düzenle: admin formu (mobile-web konum-ekle sihirbazının admin karşılığı, tek sayfada — adım adım olmasına gerek yok), `source='admin'`, `status='published'` doğrudan yayınlanabilir
- [ ] Excel/CSV toplu içe aktarma: şablon indirme (kolon başlıkları), dosya yükleme + satır doğrulama (zorunlu alanlar, kategori/enum değerleri) + toplu insert (`source='import'`)
- [ ] Hata Bildirimi Kuyruğu: `reports` tablosu listesi (open/reviewing/resolved filtre), durum güncelleme — bu güncelleme zaten Faz 7'deki `notify_on_report_status_change` trigger'ını otomatik tetikleyecek
- [ ] Kullanıcı Yönetimi: kullanıcı listesi, `tier` görüntüleme/manuel değiştirme (free/premium), hesap askıya alma (yeni bir `profiles.suspended` kolonu gerekebilir — migration gerekirse ekle)
- [ ] Yorum Moderasyonu: uygunsuz yorumları listeleyip silme (`reviews` tablosu, admin DELETE RLS'i kontrol/gerekirse ekle)
- [ ] İçerik Yönetimi: "Kamp.la Hakkında"/Kullanım Koşulları/Gizlilik metinlerini düzenleme — mobile-web'deki statik sayfalar şu an kod içine gömülü metin; bu görev için basit bir `site_content` tablosu (key/value/lang) migration'ı gerekebilir, admin'den düzenlenip mobile-web'in aynı tablodan okuması ayrı bir alt görev
