...
## KRİTİK NOT — gerçek ikon/logo export'ları `design/icons-export/` altında (2026-07-26)

**Figma MCP quota'sı dolu olsa bile, ikon/logo işine başlamadan önce HER ZAMAN
önce `design/icons-export/` klasörüne bak.** Bu klasörde Mustafa'nın kendisi
Figma'dan export ettiği 118 GERÇEK SVG dosyası var (logo, pin, filter, poi
glifileri — çadır/karavan/park/su/çamaşır/duş —, sosyal medya ikonları, bayrak
ikonları, vb.) — ayrıca `design/*.png` altında 44 ekranın tam ekran görüntüleri
de var (hangi ikonun hangi ekranda/renkte kullanıldığını görsel olarak
doğrulamak için).

Bu not, 2026-07-25/26 otonom oturumlarında yaşanan ciddi bir hatayı önlemek
için eklendi: Figma MCP `download_assets`/`get_design_context` çağrıları
kotaya takılınca, önceki bir oturum bu gerçek export klasörünü FARK ETMEDEN
Ana Ekran/Liste/Filtre/Arama ekranlarındaki ikonları ve marka logosunu
screenshot'a bakarak ELLE ÇİZİLMİŞ yaklaşık SVG path'leriyle kodladı — halbuki
gerçek export'lar zaten diskte duruyordu. Mustafa bunu fark edip düzeltilmesini
istedi (bkz. STATUS.md 2026-07-26 "Gerçek Figma ikon/logo entegrasyonu" kaydı).
**Figma MCP'ye hiç ihtiyaç duymadan bu klasörden ikon/logo path'i almak her
zaman mümkünse tercih edilmeli** — MCP quota'sı sadece layout/spacing/renk
token'ı gibi ikon dışı detaylar için gerekli olabilir.

Tek bilinen eksik: `search` (büyüteç) ikonu için ayrı bir export dosyası yok
(118 dosya arasında yok) — bu tek node için Mustafa'nın ayrıca export etmesi
gerekiyor (bkz. BLOCKERS.md).
